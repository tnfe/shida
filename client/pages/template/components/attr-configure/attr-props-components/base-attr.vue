    ...
    <el-collapse v-model="activeNames">
      ...
      <el-collapse-item title="字体：" name="4">
        ...
        <div class="random-color">
            <el-checkbox v-model="textColorIsRandom" class="el-checkbox">随机</el-checkbox>
            <el-select v-model="activeElement.commonStyle.candidateColor" multiple :disabled="!textColorIsRandom" 
            @change="setTextRandomColor" placeholder="请选择随机列表" class="el-select">
              <el-option
                v-for="item in popularTextColors"
                :key="item.value"
                :label="item.name"
                :value="item.value">
              </el-option>
...
export default {
  data() {
    return {
      popularTextColors:[{name:'橙红色', value:'#FF2400'},
      {name:'淡紫色', value:'#DB70DB'}, 
      {name:'中春绿色', value:'#7FFF00'}, 
      {name:'霓虹粉红', value:'##FF6EC7'}, 
      {name:'石板蓝', value:'#007FFF'}, 
      {name:'青色', value:'#00FFFF'}, 
      {name:'珊瑚红', value:'#FF7F00'}],
      activeNames: ["1"],
      alignTypeList: [
        {
          title: "左对齐",
  ...
  watch: {
    ...
    activeNames() {
      this.$store.commit("template/updateActiveAttrEditCollapse", this.activeNames);
    }
  },
  created() {
  ...
  methods: {
    ...
    addHistory() {
      ...
      this.$store.dispatch("template/addHistoryCache");
    },
    /**
     * 随机设置字体颜色
     * */
    setTextRandomColor() {
      const TextRandomColorLength = this.activeElement.commonStyle.candidateColor.length
      if(TextRandomColorLength > 0) {
        this.activeElement.commonStyle.color = this.activeElement.commonStyle.candidateColor[Math.floor(Math.random()*TextRandomColorLength)];
      }
      return;
    },
    /**
