...
export default {
  ...
  computed: {
    ...
    ...mapGetters('template', ["currentPageIndex", "activeElementIndex", "activeElement"])
  },
  watch: {
    activePageUUID() {
      // 关闭选择动画弹窗
  ...
  methods: {
    ...
    handleChooseAnimate(item) {
      ...
      if (this.reSelectAnimateIndex === undefined) {
        ...
        this.$store.dispatch("template/addElementAnimate", item.value);
      } else {
    ...
    handleDeleteAnimate(index) {
      this.$store.dispatch("template/deleteElementAnimate", index);
    },
    ...
    // saveRandomAnimate(){
    //   this.showAnimatePanel = false;
    //   const randomAnimate = this.createRandomAnimate(this.activeName)
    //   this.handleChooseAnimate(randomAnimate)
    // }
  }
};
</script>
