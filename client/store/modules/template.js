...
import editorProjectConfig from "@client/pages/template/DataModel";
...
const actions = {
  ...
  copyElement({ state, commit }, elData) {
    ...
    let activePage = getters.activePage(state);
    let data = editorProjectConfig.copyElement(copyOrignData, { zIndex: activePage.elements.length + 1 },"element");
    commit("addElement", data);
    commit("setActiveElementUUID", data.uuid);
    commit("addHistoryCache");
  },
...
const mutations = {
  ...
  deletePage(state, index) {
    // console.log("current:",getters.currentPageIndex(state))
