...
export default {
  ...
  methods: {
    ...
    handleElementClick(uuid) {
      // console.log("点击")
      this.$store.dispatch("template/setActiveElementUUID", uuid);
    },
    /**
     * 移动改变元素大小定位
    ...
    handleClickCanvas(e) {
      if (
        ...
        this.$store.dispatch("template/setActiveElementUUID", "");
      }
    },
    /**
     * 对元素进行操作命令
    ...
    handleElementCommand(command) {
      this.$store.dispatch("template/elementCommand", command);
    },
    /**
     * 监听键盘事件
     * @param e
